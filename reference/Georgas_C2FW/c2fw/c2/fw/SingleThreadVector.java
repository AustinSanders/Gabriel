/*
 * @(#)SingleThreadVector.java	1.40 00/06/27
 * 
...
/**
 * The <code>SingleThreadVector</code> class implements a growable array of 
 * objects. Like an array, it contains components that can be 
 * accessed using an integer index. However, the size of a 
 * <code>SingleThreadVector</code> can grow or shrink as needed to accommodate 
 * adding and removing items after the <code>SingleThreadVector</code> has been created.
 * <p>
 ...
 * <p>
 * This implementation of <code>SingleThreadVector</code> is identical to
 * <code>java.util.Vector</code> except no methods are synchronized.  Hence,
 * while this class is not thread-safe for use by more than one thread,
 * it's really fast.  If you synchronize on the object on all uses or
 * you are sure that this class is used only by one thread, this is better
 * than java.util.Vector.
 *
 * @author  Lee Boynton
 * @author  Jonathan Payne
 * @author	Eric M. Dashofy
 * @version 1.40, 06/27/00
 * @since   JDK1.0
 */
public class SingleThreadVector implements Cloneable, java.io.Serializable {
	/**
	 ...
	 * @since   JDK1.0
	 */
	public SingleThreadVector(int initialCapacity, int capacityIncrement) {
		super();
	 ...
	 * @since   JDK1.0
	 */
	public SingleThreadVector(int initialCapacity) {
	...
	public Object clone() {
		try { 
			SingleThreadVector v = (SingleThreadVector)super.clone();
...
final	class VectorEnumerator implements Enumeration {
	SingleThreadVector vector;
	int count;
	
	VectorEnumerator(SingleThreadVector v) {
		vector = v;
		count = 0;
	}
